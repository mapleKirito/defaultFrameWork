...
$(document).ready(function () {
	//判断浏览器版本
		//顶部标题栏 
	$("#navigation").width($(window).innerWidth()-192);
	...
	var frmAutoResizer = function () {
		...
		if ($("#layout_left").height() != $(window).innerHeight() - 48) {
            $("#layout_left").height($(window).innerHeight() - 48);
        }
        
		if ($("#layout_left_pms").height() != $(window).innerHeight() - 48) {
            $("#layout_left_pms").height($(window).innerHeight() - 48);
        }
       
        if ($("#layout_right").width() != windowFrmW - leftFrm ) {
            $("#layout_right").width(windowFrmW - leftFrm);
        }
        if ($("#layout_right").height() != $(window).innerHeight() - 88) {
            $("#layout_right").height($(window).innerHeight()-88);
        }
        ...
        if ($(".layout_left_menutab_body").height() != $(window).innerHeight() - topFrm - 40) {
            $(".layout_left_menutab_body").height($(window).innerHeight() - topFrm - 40);
        }
        if ($(".layout_left_menutab_pms_body").height() != $(window).innerHeight() - topFrm) {
            $(".layout_left_menutab_pms_body").height($(window).innerHeight() - topFrm);
        }
        if ($(".left_menutab_body").height() != $(window).innerHeight() - 90) {
            $(".left_menutab_body").height($(window).innerHeight() - 90);
        }
    ...
    //所有按钮渐变
	var needAnim = [".fade-button", ".login-info-more a", "#metrogrid>a", ".res-list-pager a.pre", ".res-list-pager a.next", ".res-list-pager span.pager-info input[type=button]"];
    ...
    $(".login-info").hover(function () {
    	if($(this).parent().find(".login-info-more").is(":hidden")){
    ...
    //maple系统信息下拉框
    $(".sys_info").hover(function () {
    	if($(this).parent().find("#sys_info_more").is(":hidden")){
            $(this).parent().find("#sys_info_more").slideDown();
    	}
    }, function () {
        $(this).parent().find("#sys_info_more").slideUp();
    });
    ...
    $(".gallery").hover(function () {
        ...
        var span = title.find("span");
        var icons = title.find("p");
        TweenLite.to(title, needAnimTimer, { backgroundColor: "#7c6d5b", opacity: 0.7 });
    ...
    $(".relative-res span.toggle").click(function () {
        if ($(".relative-res-body").height() > 0) {
            ...
            $(".relative-res-title").find(".tabInfo").hide();
            
        } else {
            $(this).addClass("on");
            $(".relative-res-body").animate({ height: 150 }, 500);
            $(this).find("a>b").text("收缩");
            var index = $(".relative-res-title a.tab").index($(".relative-res-title a.on"));
            $(".relative-res-title").find(".tabInfo").hide().eq(index).show();
        }
    });
    ...
    $(".relative-res-title a.tab").click(function () {
        if ($(".relative-res-body").height() <= 0) {
            ...
            $(".relative-res-body").animate({ height: 150 }, 500);
            //alert($(".toggle").find("a>b").html());
            //$(this).find("a>b").text("收缩");
            $(".toggle").find("a>b").text("收缩");
        }
    ...
    $("#btn_scale a").click(function () {
        ...
        if (leftWidth == 0) {
            ...
            $("#navigation").stop(true,true).animate({ left: 192 });
            $("#navigation").show().animate({ width: ($(window).innerWidth()-192) }, 500)
            $(this).html("&lt;");
			$("#layout_left").animate({ width: 192 }, 500);
			$("#layout_left_pms").animate({ width: 192 }, 500);
    ...
    $("#layout_left_pms").mouseleave(function () {
        if ($("#div_sub_submenu").is(":visible")) {
            setTimeout(function () {
                if ($("#div_sub_submenu").attr("isOnMouseOver") == undefined) {
                    $("#div_sub_submenu").hide();
    ...
    $(".relative-res-title a.tab").click(function () {
        ...
        var index = $(".relative-res-title a.tab").index(this);
        $(".relative-res-body").find(".relative-res-container").hide().eq(index).show();
        $(".relative-res-body").find(".controll").hide().eq(index).show();
    });
    ...
    //图片资源切换 控制柄
      $(".relative-res-body .btn").click(function () {
          ...
          if ($("#relative-res-body_res_t").attr("data-count") == undefined) {
          	
          	$("#relative-res-body_res_t").attr("data-count", $("#relative-res-body_res_t li").length);
             
          }
          if ($("#relative-res-body_res_t").attr("data-index") == undefined) {
          	$("#relative-res-body_res_t").attr("data-index", 1);
          }
         
          var count = parseInt($("#relative-res-body_res_t").attr("data-count"));
          var index = parseInt($("#relative-res-body_res_t").attr("data-index"));
          var container = $("#relative-res-body_res_t");
          var type = $(this).attr("data-type");
          var indexPlus = function () {
              if (index + 1 > count) {
              	$("#relative-res-body_res_t").attr("data-index", count);
                  return false;
              } else {
              	$("#relative-res-body_res_t").attr("data-index", index + 1);
                  return true;
              }
          };
       ...
       //相关资源切换 控制柄
      $(".relative-res-body .btn_x").click(function () {
    
          if ($("#relative-res-body_res_x").attr("data-count") == undefined) {
          	
          	$("#relative-res-body_res_x").attr("data-count", $("#relative-res-body_res_x li").length);
             
          }
          if ($("#relative-res-body_res_x").attr("data-index") == undefined) {
          	$("#relative-res-body_res_x").attr("data-index", 1);
          }
         
          var count = parseInt($("#relative-res-body_res_x").attr("data-count"));
          var index = parseInt($("#relative-res-body_res_x").attr("data-index"));
          var container = $("#relative-res-body_res_x");
          var type = $(this).attr("data-type");
          var indexPlus = function () {
              if (index + 1 > count) {
              	$("#relative-res-body_res_x").attr("data-index", count);
                  return false;
              } else {
              	$("#relative-res-body_res_x").attr("data-index", index + 1);
                  return true;
              }
          };
          var indexMinus = function () {
      ...
      $('.layout_left_menutab_body,#layout_mainbody,.reviewC,.left_menutab_body,#layout_left_pms,#createtable9').rollbar({ zIndex: 10, pathPadding: "0px" });
...
//相关资源跳转
function otherRes(this_){
	//alert(365);
		//window.open(forwardUrl);
	window.parent.tabClickHref($(this_).attr("hreft"));
}
...
function isinfor(params){
	var id = "",room = "";
		...
		}else if(params.indexOf("expand") > -1){
			id = "erID";room = "expand";
				...
				success:function(jsonData,textStatus){
					if (textStatus == "success"){
						if($(".roomname").val() == 'exhibition'){
						      ...
						      //初始化 二级分类 tr
						      //alert(jsonData.ER_Secound);
						      if(isNull(jsonData.ER_Secound)){
						    	  $(".erSecound").parent().show();
						    	  $(".erSecound").html(jsonData.ER_Secound);
						      }else{
						    	  $(".erSecound").parent().hide();
						      }
						      ...
						      //初始化 四级分类 tr
						      //alert(jsonData.ER_ManyClimate);
						      if(isNull(jsonData.ER_ManyClimate)){
						    	  $(".erManyClimate").parent().show();
						    	  
						    	  var clival = "";
						    	  if(jsonData.ER_JieID.indexOf("51")>-1){
						    		  if(jsonData.ER_ManyClimate.indexOf("57")>-1){ clival+="热带&nbsp;";}
						    		  if(jsonData.ER_ManyClimate.indexOf("58")>-1){ clival+="亚热带&nbsp;";}
						    		  if(jsonData.ER_ManyClimate.indexOf("59")>-1){ clival+="温带&nbsp;";}
						    		  if(jsonData.ER_ManyClimate.indexOf("60")>-1){ clival+="亚寒带&nbsp;";}
						    		  if(jsonData.ER_ManyClimate.indexOf("61")>-1){ clival+="寒带&nbsp;";}
						    		  if(jsonData.ER_ManyClimate.indexOf("993")>-1){ clival+="高原山地气候&nbsp;";}
						      	  }
							      if(jsonData.ER_JieID.indexOf("50")>-1){
							    	  if(jsonData.ER_ManyClimate.indexOf("52")>-1){ clival+="热带&nbsp;";}
						    		  if(jsonData.ER_ManyClimate.indexOf("53")>-1){ clival+="亚热带&nbsp;";}
						    		  if(jsonData.ER_ManyClimate.indexOf("54")>-1){ clival+="温带&nbsp;";}
						    		  if(jsonData.ER_ManyClimate.indexOf("55")>-1){ clival+="亚寒带&nbsp;";}
						    		  if(jsonData.ER_ManyClimate.indexOf("56")>-1){ clival+="寒带&nbsp;";}
						    		  if(jsonData.ER_ManyClimate.indexOf("992")>-1){ clival+="高原山地气候&nbsp;";}
							      }
						    	  $(".erManyClimate").html(clival);
						      }else{
						    	  $(".erManyClimate").parent().hide();
						      }
						     ...
						     //初始化 中文门名 tr
						      ...
						      if(isNull(jsonData.ER_Country_Name)){
						    	  $(".erCountryName").parent().show();
						    	  $(".erCountryName").html(jsonData.ER_Country_Name);
						      }else{
						    	  $(".erCountryName").parent().hide();  
						      }
						       //初始化 省份 tr
						      //alert("ER_ProvinceName=="+jsonData.ER_ProvinceName);
						      if(isNull(jsonData.ER_Province_Name)){
						    	  $(".erProvinceName").parent().show();
						    	  $(".erProvinceName").html(jsonData.ER_Province_Name);
						      }else{
					    	 ...
					    	 $(".addFavorite").attr("id","-701007|"+jsonData.ER_Path+"|expand|1"+"|"+jsonData.ER_Name+"|1011|"+serid);
					    	 
					 		  $("#download").attr("href",'downloadAction.action?code=-706&paramMap.resPath='+jsonData.ER_Upload+"&paramMap.cname="+jsonData.ER_Name+"&paramMap.ctype="+jsonData.ER_Type+"&paramMap.cid="+jsonData.ER_Type+"&paramMap.croom=expand");
					 		  //初始化 资源编号 tr
					 		  //alert(jsonData.ER_No);
						      $(".erNo").html(jsonData.ER_No);
					 		  //初始化 资源名称 tr
						      //alert(jsonData.ER_Name);
						      $(".erName").html(jsonData.ER_Name);
						      //初始化 资源分类 tr
						      //alert(jsonData.ER_ResTypeName);
						      $(".erResTypeName").html(jsonData.ER_ResTypeName);
						      //初始化 二级分类 tr
						      //alert(jsonData.ER_CateTwLevel_Name);
						      if(isFieldNull(jsonData.ER_CateTwLevel_Name)){
						    	  $(".erCateTwLevelName").parent().show();
						    	  $(".erCateTwLevelName").html(jsonData.ER_CateTwLevel_Name);
						      }else{
						    	  $(".erCateTwLevelName").parent().hide();
						      }
						      //初始化 详细信息 tr
						      //alert(jsonData.ER_Content);
						      if(isFieldNull(jsonData.ER_Content)){
						    	  $(".erContent").parent().show();
						    	  $(".erContent").html(jsonData.ER_Content);
						      }else{
						    	  $(".erContent").parent().hide();
						      }
					    	 ...
					    	 $(".addFavorite").attr("id","-701007|"+jsonData.CR_Path+"|lovely|1"+"|"+jsonData.CR_Name+"|1011|"+serid);
					 		  ...
					 		  $("#download").attr("href",'downloadAction.action?code=-706&paramMap.resPath='+jsonData.CR_Upload+"&paramMap.cname="+jsonData.ER_Name+"&paramMap.ctype="+jsonData.CR_Type+"&paramMap.cid="+jsonData.CR_Type+"&paramMap.croom=lovely");
					 		  //初始化 资源编号 tr
					 		  //alert(jsonData.ER_No);
...
function BigImg(flag){

	var forwardUrl = "decorateForwardAction.action?code=002030";
	var roomType = $("#roomname").val();
	var id = $("#reviewID").val();
	forwardUrl += "&paramMap.roomtype="+roomType+"&paramMap."+flag+"ID="+id+"&suiji="+new Date().getTime();
	forwardUrl += "&paramMap.src="+$("#resView").attr("src")+"&paramMap.upload="+$("#resView").attr("upload");
	window.open(forwardUrl);
}
function preImg(flag,this_){
	
	if(flag == 0){
		//向前翻页图片
		var currentIndex = $("#resView").attr("_index");
		var allPic = $("#relative-res-body_res_t li").length;
		...
		if(currentIndex==0){
			...
			var upload = $("#relative-res-body_res_t li").eq(parseInt(currentIndex)-1).find("a").attr("upload");			
			var inthum = $("#relative-res-body_res_t li").eq(parseInt(currentIndex)-1).find("a").attr("inthum");
		   	   var code= $("#relative-res-body_res_t li").eq(parseInt(currentIndex)-1).find("a").attr("code");
			   var name= $("#relative-res-body_res_t li").eq(parseInt(currentIndex)-1).find("a").attr("name");
			var params = $("#relative-res-body_res_t li").eq(parseInt(currentIndex)-1).find("a").attr("params");
			 var audio = $("#relative-res-body_res_t li").eq(parseInt(currentIndex)-1).find("a").attr("audio");
		  var position = $("#relative-res-body_res_t li").eq(parseInt(currentIndex)-1).find("a").attr("position");
			
			//同步地图定位图片
			if(position != ''){
				$(".rc-position").parent().show();
				$("#erCityName").val(position);
			}else{
				$(".rc-position").parent().hide();
			}
		
			//同步位置信息
			
			
			var titleMsg=$("body",parent.document).find('.menuTabTextNavigation').html();
			
			var newTitle=titleMsg.substring(0, titleMsg.lastIndexOf("&gt;")+4)+name;
		    $("body",parent.document).find('.menuTabTextNavigation').html(newTitle);
			//同步名称
			$(".res-detail-info h4 label").html(name);
			//同步评论信息
			$("#reviewID").val($("#relative-res-body_res_t li").eq(parseInt(currentIndex)-1).find("a").attr("reviewID"));
			//$("#revL").load("decorateForwardAction.action?code=-600604&paramMap.ptype="+$("#roomname").val()+"&paramMap.pid="+$("#reviewID").val()+"&paramMap.resURL="+$(".addFavorite").attr("id")+"&paramMap.userID=${paramMap.s_userID}&paramMap.userType=${paramMap.s_userType}");
			...
			//同步音频
			if(audio != ''){
				$("#mp").parent().parent().show();
				$("#mp").attr("src",audio);
			}else{
				$("#mp").parent().parent().hide();
			}
		...
		//
		var currentIndex = $(this_).attr("_index");
		var allPic = $("#relative-res-body_res_t li").length;
		
			//需要将数据依次向后展示
			$("#resView").attr("_index",parseInt(currentIndex));
			var upload = $("#relative-res-body_res_t li").eq(parseInt(currentIndex)).find("a").attr("upload");			
			var inthum = $("#relative-res-body_res_t li").eq(parseInt(currentIndex)).find("a").attr("inthum");
			var code= $("#relative-res-body_res_t li").eq(parseInt(currentIndex)).find("a").attr("code");
			var name= $("#relative-res-body_res_t li").eq(parseInt(currentIndex)).find("a").attr("name");
			var params = $("#relative-res-body_res_t li").eq(parseInt(currentIndex)).find("a").attr("params");
			var audio = $("#relative-res-body_res_t li").eq(parseInt(currentIndex)).find("a").attr("audio");
			var position = $("#relative-res-body_res_t li").eq(parseInt(currentIndex)).find("a").attr("position");
			//同步地图定位图片
			if(position != ''){
				$(".rc-position").parent().show();
				$("#erCityName").val(position);
			...
			var newTitle=titleMsg.substring(0, titleMsg.indexOf($("#erTotal").val())+$("#erTotal").val().length+1)+">"+name;
			
			$("body",parent.document).find('.menuTabTextNavigation').html(newTitle);
			//同步名称
			$(".res-detail-info h4 label").html(name);
			//同步评论信息
			$("#reviewID").val($("#relative-res-body_res_t li").eq(parseInt(currentIndex)).find("a").attr("reviewID"));
			//$("#revL").load("decorateForwardAction.action?code=-600604&paramMap.ptype="+$("#roomname").val()+"&paramMap.pid="+$("#reviewID").val()+"&paramMap.resURL="+$(".addFavorite").attr("id")+"&paramMap.userID=${paramMap.s_userID}&paramMap.userType=${paramMap.s_userType}");
