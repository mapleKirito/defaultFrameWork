...
$(document).ready(function(e) {
		...
		var CalendarHandler = {
			...
			initialize: function() {
				...
				var cm_height = parseInt($("#CalendarMain").css("height"));
				
				$("#context").css("height", cm_height + "px");
				var ct_height = parseInt($("#context").css("height"));
				var ct_width = parseInt($("#context").css("width"));
				$("#center").css("height", ct_height - 30 + "px");
				$("#selectYearDiv").css("height", ct_height - 30 + "px").css("width", ct_width + "px");
				$("#selectMonthDiv").css("height", ct_height - 30 + "px").css("width", ct_width + "px");
				$("#centerCalendarMain").css("height", ct_height - 30 + "px").css("width", ct_width + "px");

				$calendarItem.css("height", ct_height - 30 + "px"); //.css("visibility","hidden");
				$("#Container").css("height", "0px").css("width", "0px").css("margin-left", ct_width / 2 + "px").css("margin-top", (ct_height - 30) / 2 + "px");
				$("#Container").animate({
					width: ct_width + "px",
					height: (ct_height - 30) * 2 + "px",
					marginLeft: "0px",
					marginTop: "0px"
				}, 300, function() {
					$calendarItem.css("visibility", "visible");
				});
				$(".dayItem").css("width", ct_width + "px");
				var itemPaddintTop = parseInt($(".dayItem").css("height")) / 6;
				$(".item").css({
					"width": parseInt($(".week").css("width")) / 7 + "px",
					"line-height": (itemPaddintTop-15 )+ "px",
					"height":( itemPaddintTop-15) + "px"
				});
				$(".currentItem>a").css("margin-left", (parseInt($(".item").css("width")) - 25) / 2 + "px").css("margin-top", (parseInt($(".item").css("height")) - 25) / 2 + "px");
				$(".week>h3").css("width", parseInt($(".week").css("width")) / 7 + "px");
				this.RunningTime();
			},
			...
			//	$(".currentDay").show();
				...
				$("#selectMonthDiv>div").css("width", ($("#center").width() - 4) / 6 + "px").css("line-height", ($("#center").height() - 4) / 5 + "px");
				$("#centerMain").animate({
			...
			CreateCalendar: function(y, m, d) {
				...
				if (nowWeek != 0) {
					...
					for (var i = (lastMonthDaysNub - (nowWeek - 1)); i < lastMonthDaysNub; i++) {
						$dayItem.append("<div class=\"item lastItem\"><a href=\"javascript:void(0);\" >" + (i + 1) + "</a></div>");
					}
				}

				//生成当月的日期
				for (var i = 0; i < nowDaysNub; i++) {
					if (i == (nowDay - 1)) $dayItem.append("<div onclick='history(this," + (i + 1) +")' class=\"item currentR \"><a  href=\"javascript:void(0);\" class='currentitem'>" + (i + 1) + "</a></div>");
					else $dayItem.append("<div onclick='history(this,"+ (i + 1) +")' class=\"item\"> <a href=\"javascript:void(0);\">" + (i + 1) + "</a></div>");
				}
			...
			CSS: function() {
				...
				$(".item").css({
					...
					"line-height": (43.3333333333333) + "px",
					"height": (43.3333333333333) + "px"
				});
	...
	function history(_this,day){
			var dath  = day+'';
		//	$(".item").find('a').removeClass("current");
			//$(".item").find('a').removeClass("currentitem");
		//	$(_this).find("a").addClass("current");
			var manth = $(".selectMonth").html();
			manth = manth.replace ('月','');
			...
			if(dath.length != 2)
				dath = '0'+dath;
			window.parent.tabClick("9011","历史上的今天", '9011','paramMap.historyNum=-'+manth+'-'+dath+'&paramMap.findMonth='+findMonth+'&paramMap.findDay='+findDay);	
...
/*
		...
		var CalendarHandler = {
			...
			initialize: function() {
				...
				$(".item").css({
					"width": $(".week").width() / 7 + "px",
					"line-height": (itemPaddintTop - 20) + "px",
					"height": (itemPaddintTop - 20) + "px"
				});
				$(".currentItem>a").css("margin-left", ($(".item").width() - 25) / 2 + "px").css("margin-top", ($(".item").height() - 25) / 2 + "px");
			...
			CreateCalendar: function(y, m, d) {
				...
				var nowYear = y == 0 ? nowDate.getFullYear() : y;
				this.currentYear = nowYear;
				var nowMonth = m == 0 ? parseInt($("#findMonth").val())  : m;
				this.currentMonth = nowMonth;
				var nowDay = d == 0 ?  parseInt($("#findDay").val()) : d;
				...
				//生成当月的日期
				for (var i = 0; i < nowDaysNub; i++) {
					if (i == (nowDay - 1)) $dayItem.append("<div onclick='history(this," + (i + 1) +")' class=\"item currentR \"><a class='currentitem'>" + (i + 1) + "</a></div>");
					else $dayItem.append("<div onclick='history(this,"+ (i + 1) +")' class=\"item\"><a>" + (i + 1) + "</a></div>");
				}
