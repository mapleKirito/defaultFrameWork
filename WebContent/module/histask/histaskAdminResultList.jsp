...
$(document).ready(function(){
	...
	var ujdata = {
    	...
    	shortcut:{
	    	...
	    	width:'10%',
	    	event:[
				{operate:'preview',name:'${pmsIndex.sys_result_param_review.name}',disabled:'false',isvalid:'${!empty pmsIndex.sys_result_param_review}',param:'code=${pmsIndex.sys_result_param_review.code}'}
      		]
	    },
    	columnDesc:{
	    	//列表要显示的列（在json中对应的key）
			showColumn: ['htaskTitle','htaskKeywords','htaskIsReview','htaskCreatetime'],
			//列表头显示名称
			columnName:   ['中文名','关键词','状态','创建时间'],
			//列表头宽度
			columnWidth:   ['43%','22%','10%','10%'],
    	...
    	ajax:{
        	...
        	data:'code=${pmsIndex.sys_result_param_search.code}'
	    },
	    //是否有分页工具条
	    isPagination:'true',	    
    	paginationData:{
    ...
    $('#grid').gridTB(ujdata, ujoption);
   	$("#search_").click(function(){
   		//alert('${pmsIndex.sys_result_param_search.code}');
    	...
    	windowDocument:window.parent.parent.document,
   		...
   		clickEvent:function(data){
   			if ($("#grid tbody input[type='checkbox']:checked").size() == 0){
   				...
   				$("#actionForm").attr("action",faction +"?code=${pmsIndex.sys_result_param_review.code}");
   				$("#actionForm").submit();   
   			}
   		}
   	});
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="SkyTableLine">
				...
				<td align="left">
					...
					<div class="lt-command">
	 					<s:if test="pmsIndex.sys_result_param_review">
	 						<a id="close_histask" href="javascript:void(0)">
					 			<span>${pmsIndex.sys_result_param_review.name}</span>
					 		</a>
					 	</s:if>
					 	<div style="float: right;margin-right:15px;">
					 		<span>名称：</span>
			       		    <input type="text" name="searchMap.htaskTitle" value="${paramMap.htaskTitle}" id="htaskTitle" style="width: 200px;" />
			       		    <s:if test="pmsIndex.sys_result_param_search">
							<a href="javascript:void(0)" id="search_"><span>${pmsIndex.sys_result_param_search.name}</span></a>
							</s:if>
					 	</div>
				 	</div>
				</td>
