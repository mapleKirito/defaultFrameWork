...
<input type="hidden" id="changeType" value="${paramMap.changeType }">
	<div id="layout_mainbody">
		...
		<div class="page-container">
			...
			<s:if test="initMap.gradeView.isEmpty()">
				<center>
			...
			<form name="actionForm" method="post"
				...
				>
				<s:if test="initMap.gradeView.isEmpty()== false">

					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						class="table_to res_info" id="funbig1" style="display: none;">
						<tr>
							<s:iterator value="initMap.gradeView" id="fv" status="fvindx">
								...
								<td align="center">
									<div id="boxgrid<s:property value=" #fvindx.index " />" class="gallery" style="position: relative; top: 5px;">
										<div style="height: 180px; background: #e8ddd0;" id="outdiv">
											<s:set name="pcode" value="''" />
											<s:set name="pname" value="''" />
											<s:set name="expurl" value="''" />
											<s:if test="#fv.grResourceType == 'exhibition'">
												<s:set name="pcode" value="'-901001'" />
												<s:set name="pname" value="'erID'" />
												<s:set name="broomname" value="'er'" />
												<s:if test="#fv.grType == '1011'">
													<s:set name="pcode" value="'-901001'" />
												</s:if>
											</s:if>
											<s:if test="#fv.grResourceType == 'observation'">
												<s:set name="pcode" value="'-901004'" />
												<s:set name="pname" value="'orID'" />
												<s:set name="broomname" value="'or'" />
												<s:if
													test="#fv.grType == '1011' || #fv.grType == '1022' || #fv.grType == '2055'">
													<s:set name="pcode" value="'-901004'" />
												</s:if>
												<s:if test="#fv.grType == '4033' ">
													<s:set name="pcode" value="'-901004'" />
												</s:if>
												<s:if
													test="#fv.grType == '1084'">
													<s:set name="pcode" value="'-901005'" />
												</s:if>
												<s:if
													test="#fv.grType == '2041' || #fv.grType == '2033' ">
													<s:set name="pcode" value="'-701019'" />
												</s:if>
											</s:if>
											<s:if test="#fv.grResourceType == 'laboratory'">
												<s:set name="pcode" value="'-901007'" />
												<s:set name="pname" value="'lrID'" />
												<s:set name="broomname" value="'lr'" />
												<s:if
													test="#fv.grType == '1011' || #fv.grType == '1022'||#fv.fileType == '4033' || #fv.grType == '2055'">
													<s:set name="pcode" value="'-901007'" />
												</s:if>
												<s:if
													test="#fv.grType == '1084'">
													<s:set name="pcode" value="'-901008'" />
												</s:if>
												<s:if test="#fv.grType == '2002'">
													<s:set name="pcode" value="'-901009'"></s:set>
												</s:if>
												<s:if
													test="#fv.grType == '2041' || #fv.grType == '2033' ">
													<s:set name="pcode" value="'-901018'" />
												</s:if>
											</s:if>
											<s:if test="#fv.grResourceType == 'projection'">
												<s:set name="pcode" value="'-901011'" />
												<s:set name="pname" value="'prID'" />
												<s:set name="broomname" value="'pr'" />
												<s:if test="#fv.grType == '1084'">
													<s:set name="pcode" value="'-901011'" />
												</s:if>
												<s:if test="#fv.grType == '1073'">
													<s:set name="pcode" value="'-901022'" />
												</s:if>
											</s:if>
											<s:if test="#fv.grResourceType == 'lovely'">
												<s:set name="pcode" value="'-901014'" />
												...
												<s:if
													test="#fv.grType == '1084'">
													<s:set name="pcode" value="'-901015'" />
												</s:if>
												<s:if test="#fv.grType == '2095' ">
													<s:set name="pcode" value="'-901020'" />
												</s:if>
												<s:if
													test="#fv.grType == '2041' || #fv.grType == '2033' ">
													<s:set name="pcode" value="'-901020'" />
											...
											<s:if test="#expurl == ''">
												<a href="javascript:void(0)" code="${pcode}"
													...
													param="&paramMap.grGrade=${fv.grGrade}&paramMap.path=${fv.path}&paramMap.${broomname}Type=${fv.grType}&paramMap.${pname}=${fv.grResourceID}&paramMap.rrType=${fv.grType}&paramMap.st=3&paramMap.temppname=${pname}&paramMap.temppid=${fv.grResourceID}&paramMap.name=${fv.grName}&paramMap.roomtype=${fv.grResourceType}">
													<img src="${fv.grThumbnail}" alt="${fv.grName}"
													style="" height="180px"
											...
											<s:if test="#expurl != ''">
												<s:if test="session.s_userType == 1">
													<a href="${expurl}&code=002001004"> <span
														style="color: #000000;"> <img src="${fv.grThumbnail}"
															alt="${fv.grName}" style="" height="180px"
															class="image eightPic"
															id="imas<s:property value=" #fvindx.index
										" />" />
													</span>
										...
										" />" />
											...
											<div class="textPdown title" id="textPdown${fvindx.index}">
												<span> <a href="javascript:void(0)"
													style="color: white;"> <s:if
															test="#fv.grName.length() > 9">
															<s:property value="#fv.grName.substring(0,9)" />
								...
							</s:if> <s:else>
															<s:property value="#fv.grName" />
														</s:else>
												</a>
												</span>
												<!-- 如果是文物博览室带音频的资源应该与提示 -->
												...
												<p>
													...
													<%-- <em class="res-type" roomtype="${fv.grResourceType}" resid="${fv.grResourceID }" ><i class="res-type-media"></i><label></label></em> --%>
											</div>
								</td>
							</s:iterator>
						</tr>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<s:iterator value="initMap.gradeView" id="fv" status="fvindx">
							...
							<s:if test="#fv.grResourceType == 'observation'">
								...
								<s:set name="pname" value="'orID'" />
								<s:set name="broomname" value="'or'" />
								<s:if test="#fv.grType == '1011' || #fv.grType == '1022'">
									<s:set name="pcode" value="'-901004'" />
									<s:set name="type" value="'高清图片'" />
								</s:if>
								<s:if test="#fv.grType == '4033' ">
									<s:set name="pcode" value="'-901021'" />
							...
							<s:if test="#fv.grResourceType == 'laboratory'">
								...
								<s:if
									test="#fv.grType == '2041' || #fv.grType == '2033' || #fv.grType == '2055'">
									<s:set name="pcode" value="'-901009'" />
					...
					#fv.grType " escape="false " />.gif"
								...
								<td width="45%" class="onlyex" align="left">&nbsp;&nbsp; <a
									onclick="window.parent.frames.showView(this)" name="${fv.grName}"
									codeNum="${pcode}"
									param="&paramMap.path=${fv.path}&paramMap.${broomname}Type=${fv.grType}&paramMap.${pname}=${fv.grResourceID}&paramMap.rrType=${fv.grType}&paramMap.st=3&paramMap.temppname=${pname}&paramMap.temppid=${fv.grResourceID}&paramMap.name=${fv.grName}&paramMap.roomtype=${fv.grResourceType}">
										<span> <s:property value="#fv.grName" />
									</span>
								</a>
								</td>
								<td width="10%" align="center" class="onlyex"><s:if
										test="#fv.createTime.length() > 9">
										<s:property value="#fv.createTime.substring(0,9)" />
									</s:if></td>
								<td width="25%" class="onlyex" align="left">&nbsp; <s:if
										...
										<a class="lt-button fade-button"
											...
											预览 </a>
										<a class="lt-button fade-button"
											href="downloadAction.action?code=-706&paramMap.resDownRoom=${fv.grType}&paramMap.resDownType=${fv.grType}&paramMap.resPath=${fv.grUpload}">
											资源下载 </a>
									</s:if> <s:if
										test="#fv.grAudio!= '' and #fv.grAudio!= null   and #fv.grType == '1011'">

										<a
											href="downloadAction.action?code=-706&paramMap.resDownRoom=${fv.grType}&paramMap.resDownType=${fv.grType}&paramMap.resPath=<s:property value="
						#fv.grAudio " />"
