...
function Change(_this){
	...
	if(_this.checked==true){
		...
		var erOrder =  $('#erOrderID').val();
		var erCategory =  $('#erCategoryID').val();
		...
		keyWords = encodeURI(keyWords); 
		art.dialog.data('contextPath', '<%=request.getContextPath()%>');
		art.dialog.open('<%=request.getContextPath()%>/forwardAction.action?code=-110001&paramMap.erID='+erID+'&paramMap.erTotalID='+erTotal+'&paramMap.erJieID='+erJie+'&paramMap.erGangID='+erGang+'&paramMap.erChorID='+erChor+'&paramMap.keyWords='+keyWords+'&paramMap.roomtype='+$("#roomtype").val()+'&paramMap.resType=1011&paramMap.offsetAbout=10&paramMap.start=0',{width:1000,height:574,title:'资源推荐',lock:'true',background:'#000000',opacity: '0.15'}) ;
...
<input type="hidden" id="roomtype" value="exhibition">
<input  type="hidden" name="paramMap.erIsShare" type="hidden" value="1"/>
