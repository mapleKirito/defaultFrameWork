...
$(document).ready(function(){	
	...
	$(".rc-qrcode").click(function(){
		...
		//var st=localhostPaht+"/dm/show_page?id=${initMap.videoDetail.prCode}&pageName=scanner";
		var st=localhostPaht+"/dm/page/scanner/"+code+"/scanner.html";
		art.dialog.data('str', st);
		art.dialog.data('code', "${initMap.videoDetail.prCode}");
		art.dialog.data('contextPath', '<%=request.getContextPath()%>');
   ...
   $.ajax({
		url:		'ajaxAction.action?suiji='+new Date().getTime()+'&code=${code}&paramMap.roomtype=projection&paramMap.prID=${initMap.videoDetail.prID}',
		type:	    'post',
		timeout:	'60000',
		data:	Array(),
		dataType:	'json',
...
<input type="hidden" id="prName" value="${initMap.videoDetail.prName}">
<input type="hidden" id="prAudio" value="${initMap.videoDetail.prAudio}">
<input type="hidden" id="prUpload" value="${initMap.videoDetail.prUpload}">

<input type="hidden" id="prLongitude" value="${initMap.videoDetail.prLongitude}">
<input type="hidden" id="prLatitude" value="${initMap.videoDetail.prLatitude}">
...
<input type="hidden" id="userType" value="${paramMap.s_userType}">
 <div id="layout_mainbody">
 	 <div class="res-detail-info">
                    <h4><b>资源名称：</b>${initMap.videoDetail.prName}</h4>
                    <ul class="res-controls">
                        <li title="下载">
		                  	...
		                  	<div class="hide-popup popup-rc-cloud-down rc-cloud-down">
		                          ...
		                          <div class="hide-popup-content">
		                              <table>
		                                  <tr>
		                                      <td class="key keya"><a href="downloadAction.action?code=-706&paramMap.resDownRoom=projection&paramMap.resDownType=${initMap.videoDetail.prTpye}&paramMap.resPath=${initMap.videoDetail.prUpload}">资源下载</a></td>
		                                  </tr>
		                              </table>
		                          </div>
		                      </div>
		                ...
		                <s:if test="session.s_userType != 3 and paramMap.st == 1 and session.s_userType != 5">
                  			 <li>
                  			 	<a  data-type="rc-query" title="加入收藏" onclick="window.parent.insert_fav(this)" class="addFavorite rc rc-query"  href="javascript:void(0);"   id="${code}|${paramMap.path}|projection|${paramMap.st}|${initMap.videoDetail.prName}|${initMap.videoDetail.prType}|${initMap.videoDetail.prID}" >
                  				</a>  
                  			</li>
                  		</s:if>
                        ...
                        <li><a href="javascript:void(0);" title="评论" data-type="rc-comment" class="rb rc-comment" id="revR" ></a>
								...
								<input type="hidden" id="reviewID" value="${initMap.videoDetail.prID}">
							</div>
                            ...
                            <div class="hide-popup popup-rc-info">
                                ...
                                <div class="hide-popup-content">
                                 ...
                                 <tr>
										...
										<td width="75%" align="center" class="erCode">${initMap.videoDetail.prNO}</td>
									</tr>
									<tr>
										<td width="25%" align="center">资源名称</td>
										<td width="75%" align="center" class="erName">${initMap.videoDetail.prName}</td>
									</tr>
									<tr>
										<td width="25%" align="center">资源类型</td>
										<td width="75%" align="center" class="erType">
											    ${initMap.videoDetail.prType eq "1011" ? '高清图片':'' }
												${initMap.videoDetail.prType eq "1022" ? '3D图片':'' }
												${initMap.videoDetail.prType eq "1073" ? '音频':'' }
												${initMap.videoDetail.prType eq "1084" ? '高清视频':'' }
												${initMap.videoDetail.prType eq "1005" ? '动画类素材':'' }
												${initMap.videoDetail.prType eq "1006" ? '其他素材':'' }
												${initMap.videoDetail.prType eq "2041" ? 'PPT课件':'' }
												${initMap.videoDetail.prType eq "2002" ? '动画类课件':'' }
												${initMap.videoDetail.prType eq "2033" ? 'WORD课件':'' }
												${initMap.videoDetail.prType eq "2055" ? 'PDF课件':'' }
												${initMap.videoDetail.prType eq "2004" ? '其他课件':'' }
												${initMap.videoDetail.prType eq "3031" ? '环球百科':'' }
												${initMap.videoDetail.prType eq "3032" ? '文学报告':'' }
												${initMap.videoDetail.prType eq "3033" ? '论文资料':'' }
												${initMap.videoDetail.prType eq "3034" ? '资料图书':'' }
												${initMap.videoDetail.prType eq "3035" ? '适用学段、年级与大纲':'' }
												${initMap.videoDetail.prType eq "3036" ? '其他资料':'' }
										
										</td>
									</tr>
									<tr>
										<td width="25%" align="center">一级分类</td>
										<td width="75%" align="center" class="erTotal">${initMap.videoDetail.prCategoryName}</td>
									</tr>
								<tr <c:if test="${empty initMap.videoDetail.prRemarks }">style="display:none"</c:if>>
										<td width="25%" align="center">备注</td>
										<td width="75%" align="center" class="lrTotal">${initMap.videoDetail.prRemarks}</td>
									</tr>
                        ...
                        <div class="controll">
                          ...
                          <div class="res-detail-container">
                           ...
                           <div id="video" style="position:relative;margin-top: 10px;margin-left: auto;margin-right:auto;"><div id="a1"></div></div>
							...
							<script type="text/javascript">
							var upload_ = '${initMap.videoDetail.prUpload}';
							...
							CKobject.embedSWF('ckplayer/ckplayer.swf','a1','ckplayer_a1',($(window).innerHeight()-100)*1.4,$(window).innerHeight()-100,flashvars,params);
							dqbh = dqbh + 1;
							...
							/*
								...
								*/
								...
								CKobject.embedHTML5('video','ckplayer_a1',($(window).innerHeight()-100)*1.4,$(window).innerHeight()-100,video,flashvars,support);
