...
$(document).ready(function(){	
	...
	//input验证
	$.formValidator.initConfig({formid:"actionForm",onerror:function(){},onsuccess:function(){}});//表单提交 $.formValidator.initConfig({formid:"actionForm",onerror:function(){},onsuccess:function(){}});//表单提交 //表单提交 
	$("#orName").formValidator({onshow:"请输入中文名",onfocus:"请输入中文名",oncorrect:"输入正确"}).inputValidator({min:1,onerror:function(){alert("中文名不能为空,请确认");}});
    ...
    var orIsShare = '${initMap.obseInfo.orIsShare}';
    
    if(orIsShare==1){
    	//alert(erIsShare);
    	$("#orIsShare").attr('checked',true);
    }
  	//初始化推荐到首页操作
	...
	/* 
	//上传资源文件
	$("#uploadBigFile").bind('change',function(){ 
		$("#isUpload").val(null);
		$(".subButton").show();	
		$("#uploadBigFile").select();
		var realFile = $("#uploadBigFile").val() ;
		...
		if($("#"+resBeginType+"Type").val() == '1011' && fileType.toLowerCase().indexOf("jpg".toLowerCase()) > -1 || fileType.toLowerCase().indexOf("png".toLowerCase()) > -1){
			$("#obserstylemodify5").hide();
		}else{
			$("#obserstylemodify5").show();
		}
		...
		if(TypeValue==""){
			...
			$("#uploadBigFile").after($("#uploadBigFile").clone().val(""));   
			$("#uploadBigFile").remove();   
			return false;
		}else if(TypeValue=="4033"){
			if(fileType.toLowerCase().indexOf("rar".toLowerCase()) > -1 || fileType.toLowerCase().indexOf("zip".toLowerCase()) > -1){
				$(".subButton").show();	
			}else{
				alert("您上传的资源格式不正确,请确认");
			...
			if(fileType.toLowerCase().indexOf("jpg".toLowerCase()) > -1 || fileType.toLowerCase().indexOf("png".toLowerCase()) > -1 || fileType.toLowerCase().indexOf("gif".toLowerCase()) > -1){
		...
		}else if(TypeValue == '2033' || TypeValue == '2041' || TypeValue == '2055'){
				...
				if(TypeValue == '2041'){
					if(fileType.toLowerCase() == "ppt".toLowerCase()  ){
						$(".subButton").show();	
					}else{
						alert("您上传的资源格式不正确,请确认");
						$(".subButton").hide();		
		...
		if($("#orUpload").val() != $("#oldResUpload").val() && $("#orType").val() == '4033'){
			$("#orThumbnail").val(null);
			$("#obserstylemodify5 #thumrarzipmust").html("<font color=\"red\">*</font><span id=\"orThumbnailValiTip\" style=\"position:absolute;height:22px;width:270px\"></span>");;
			$("#orThumbnailVali").formValidator({onshow:"请上传资源文件",onfocus:"请上传资源文件",oncorrect:"上传正确"}).inputValidator({min:1,onerror:"资源不能为空,请确认"});
		}
							...
							<div>
								<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_9">
									<tr bgcolor="#ae9b84">
										...
										<td width="85%" class="tatd" align="left"><input name="paramMap.orName" type="text" value="${initMap.obseInfo.orName}" maxlength="180" id="orName" style="width: 200px;" />&nbsp;
									...
									<tr bgcolor="#ae9b84">
								   	...
								   	<tr bgcolor="#ae9b84">
										...
										<td width="85%" class="tatd" align="left">
											...
											&nbsp;<font color="red">*</font><span id="orTypeTip" style="position:absolute;height:22px;width:100px"></span>
									...
									<tr bgcolor="#ae9b84" id="obserstylecreate5">
									...
									<tr bgcolor="#ae9b84" id="orModelTr" style="display: none;">
										<td width="15%" class="tatd" align="right">展示模式：</td>
										<td width="85%" class="tatd" align="left" >
									...
									<tr bgcolor="#ae9b84" id="isUploadRadio">
									...
									<tr bgcolor="#ae9b84" >
										...
										<td width="85%" class="tatd" align="left" >
										<input type="checkbox"  name="paramMap.orIsShare" id="orIsShare" value="1"  >
										</td>
									</tr>
									<tr bgcolor="#ae9b84">
									...
									<%-- <tr bgcolor="#ae9b84">
										<td width="15%" class="tatd" align="right">俗名：</td>
										<td width="85%" class="tatd" align="left">
											<input type="text" name="paramMap.orCustom" id="orCustom" value="${initMap.obseInfo.orCustom}">
										</td>
								   	</tr> --%>
								   	<%-- <tr bgcolor="#ae9b84">
										<td width="15%" class="tatd" align="right">拉丁学名：</td>
										<td width="85%" class="tatd" align="left">
									...
									<tr bgcolor="#ae9b84">
										<td width="15%" class="tatd" align="right">鉴定人：</td>
										<td width="85%" class="tatd" align="left"><input name="paramMap.orIdentifier" type="text" value="${initMap.obseInfo.orIdentifier}" maxlength="64" id="orIdentifier" style="width: 200px;" />&nbsp;
									...
									<%-- <tr bgcolor="#ae9b84" id="orContent" >
									...
									<tr bgcolor="#ae9b84">
											...
											<td width="85%" class="tatd" align="left">
											<input type="checkbox" name="paramMap.grGrades" id="grGrades" value="321" <s:if test="initMap.obseInfo.orGrade.indexOf('321') > -1">checked</s:if>>必修一&nbsp;
											<input type="checkbox" name="paramMap.grGrades" id="grGrades" value="322" <s:if test="initMap.obseInfo.orGrade.indexOf('322') > -1">checked</s:if>>必修二&nbsp;
											<input type="checkbox" name="paramMap.grGrades" id="grGrades" value="323" <s:if test="initMap.obseInfo.orGrade.indexOf('323') > -1">checked</s:if>>必修三&nbsp;
											<input type="checkbox" name="paramMap.grGrades" id="grGrades" value="327" <s:if test="initMap.obseInfo.orGrade.indexOf('327') > -1">checked</s:if>>选修一&nbsp;
											<input type="checkbox" name="paramMap.grGrades" id="grGrades" value="325" <s:if test="initMap.obseInfo.orGrade.indexOf('325') > -1">checked</s:if>>选修二&nbsp;
											<input type="checkbox" name="paramMap.grGrades" id="grGrades" value="326" <s:if test="initMap.obseInfo.orGrade.indexOf('326') > -1">checked</s:if>>选修三&nbsp;
										</td>
									</tr>
 									<!-- <tr bgcolor="#ae9b84" >
...
<div id="rsyncUploadDiv" style="position:absolute;display: none;">
	...
	<form action="/dm/uploadFile/upload" id="rsyncUploadForm" enctype="multipart/form-data" method="post" target="target_upload">
		...
		<input type="button" id="rsyncSubButton" value="上传">&nbsp;
		<font color="red">*</font><span id="rsyncUploadBigFileTip" style="position:absolute;height:22px;width:297px;color:#ffffff;"></span>
	</form>
</div>
