...
$(document).ready(function(){	
	...
	var ujdata = {
	    	...
	    	columnDesc:{
				...
				showColumn: ['orName','orType','orCategoryName','orCreator'],
				//列表头显示名称
				columnName:   ['中文名','资源类型','资源分类','创建人'],
				//列表头宽度
				...
				primkeyColumn:["orID","orName","orThumbnail","orUpload","orType"],
				//要增加链接的列，可以为多个
				linkColumn:[{name:'',url:'',param:''}]
	    	},		
	    	ajax:{
	        	url:'ajaxAction.action',
	        	data:'code=${pmsIndex.res_observation_room_grade_search.code}'
		    },
		    //是否有分页工具条
		    isPagination:'true',	    
	    	paginationData:{
        ...
        $("#search_res").click(function(){
            ...
            ajax:{
                url:'ajaxAction.action',
                data:'code=${pmsIndex.res_observation_room_grade_search.code}&type=1'
                 }
            })
        	$('#grid').gridTB(ujdata);
         });
        ...
        $('#returnRes').click(function(){
    		...
    		if(chk_value.length == 0) {
    			...
    			art.dialog.data('resUpload', resUpload);//资源
    			art.dialog.data('resType', resType);//资源类型
    			art.dialog.close(); 
    		}
    	});
});
...
<input type="hidden" name="searchMap.orIsGrade" id="orIsGrade" value="0">
					...
					<td class="SkyTDLine">
						<table width="100%" border="0" cellpadding="0" cellspacing="1"
							...
							<tr align="center" class="MeNewTDLine">
								...
								<td align="left" class="SkyTDLine" width="30%">&nbsp;&nbsp;<input type="text" name="searchMap.orName" value="${paramMap.orName}" id="orName" style="width: 200px;" /></td>
								<td align="center" class="SkyTDLine" width="20%">
									<s:if test="pmsIndex.res_observation_room_grade_search">
										<a href="javascript:void(0)" id="search_res"><img src="<%=request.getContextPath()%>/${pmsIndex.res_observation_room_grade_search.icon}" align="absmiddle" width="14" border="0" height="14">${pmsIndex.res_observation_room_grade_search.name}</a>
									</s:if>
								</td>
							</tr>
						</table></td>
