...
<body>
	<div class="album-box" id="album">
		...
		<a href="images/loading.gif" title="退出查看图片" class="slide-close"></a>
	...
	<textarea class="dn" id="albumDataList">
				<li>
		            ...
		            <i class="dn" upload="${session.IP}/CacheServer/GetPicInfo?src=${paramMap.upload}">${paramMap.src}</i>
		            <i class="dn imgAlt"></i>
		        </li>
   		<s:iterator value="initMap.resView" id="fv" status="fvindx">
			<s:if test="#fv.thumbnail!=null">										
    			<li>
		            ...
		            <i class="dn" upload="${session.IP}/CacheServer/GetPicInfo?src=${fv.upload}">${session.IP}/CacheServer/GetPicInfo?src=${fv.inThum}</i>
		            <i class="dn imgAlt"></i>
		        </li>
     		</s:if>
		</s:iterator>
	...
	<script type="text/javascript">
		...
		//$("#view").click(function(){
		//	var src=$(this).attr("href")+"&r="+new Date().getTime();
		//	$(this).attr("href",src);
		//})
	</script>
</body>
